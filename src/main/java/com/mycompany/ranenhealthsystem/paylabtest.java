 ...
 * @author HP
 */
public class paylabtest extends HttpServlet {
...
if (request.getParameter("searchltest") !=null)
    {
        ...
        try
            ...
            Statement st = con.createStatement();
             ...
             String sql = "select SUM(testamount) from rhc.patientlaboratorytests where rhc.patientlaboratorytests.OPDNo = '"+OPDNO4+"'";
 ...
 while (rs.next()) {
     ...
     float total = rs.getFloat("SUM(testamount)");
     request.setAttribute("totaltest", total);
     RequestDispatcher rd;
     rd = getServletContext().getRequestDispatcher("/paymentlabtestfee.jsp");
...
if (request.getParameter("getlabdetails") !=null) 
{
    try
                {
                String OPDNO5 = request.getParameter("opdno");
               ConnectionManager db= new ConnectionManager();
            Connection con = db.getcon();
            ...
            Statement st = con.createStatement();
             ...
             String sqll = "select * from rhc.patientlaboratorytests where rhc.patientlaboratorytests.OPDNo = '"+OPDNO5+"'";
...
out.close();
  ...
  if (request.getParameter("Pay3") !=null)
    
       {
         try
            {
             String OPDNoooo = request.getParameter("opdno");       
                if ("".equals(OPDNoooo))
                {
                        out.println("<script type=\"text/javascript\">");
                        out.println("alert ('Kindly Enter OPD No. To Acertain who the patient is first!');");
                        out.println("location='paymentlabtestfee.jsp';");
                        out.println("</script>");    
                }
                else
                {     
        ...
        String OPDNoo1 = request.getParameter("opdno"); 
         String Labfee = request.getParameter("labtestfee");
...
System.out.println("AAAAAAAAAAAAAAAAAAAAAAAAAAA"+OPDNoo1+Labfee+Amnt+paymode+status);
conn=dbconn.setConnection();

 System.out.println("OOOOO"+conn);
stmt=conn.createStatement();
query= "insert into rhc.amountspaid (OPDNo,PayDetails,AmntPaid,PayMode,PayStatus)"+" values('"+OPDNoo1+"','"+Labfee+"','"+Amnt+"','"+paymode+"','"+status+"')";
