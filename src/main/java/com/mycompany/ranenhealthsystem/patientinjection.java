 ...
 * @author HP
 */
public class patientinjection extends HttpServlet {
    ...
    DatabaseConnection dbconn;
     injectioncheckservice injectioncheckservice = new injectioncheckservice();
...
//1. Adding a Injection by validating the code first on the add injection button           
            ...
            if (request.getParameter("adddinjection") !=null)
{
   try 
   
  {
           String injectid = request.getParameter("injectioncode");
                boolean authenticated = injectioncheckservice.authenticateUser(injectid);
    ...
    {                      
     ...
     ConnectionManager db= new ConnectionManager();
            ...
            Statement st = con.createStatement();
             ...
             String sql1 = "select * from rhc.injectionlist where injid = '"+injectid+"'";
        
             ResultSet rs = st.executeQuery(sql1);
     if (rs.next()) {
                 int Injectid = rs.getInt("injid");
                 String Injectname = rs.getString("injname");
                 String Injectcost = rs.getString("injcost");
    
                request.getSession().setAttribute("Injectionid", Injectid);
                request.getSession().setAttribute("Injectionname", Injectname);
                request.getSession().setAttribute("Injectioncost", Injectcost);
                
                request.getRequestDispatcher("patientinjection.jsp").forward(request, response);
                request.getRequestDispatcher("patientinjection.jsp").forward(request, response);
                request.getRequestDispatcher("patientinjection.jsp").forward(request, response);
...
if (request.getParameter("viewinjections") !=null)

{
 try
            ...
            Statement st = con.createStatement();
             ...
             String sql = "select * from rhc.injectionlist";
...
if (request.getParameter("sendinjection") !=null) 
{
    try
    {
         ...
         String injectname = request.getParameter("InjectionName");
         String injectAmount = request.getParameter("injectionprice");
         
         
           System.out.println("AAAAAAAAAAAAAAAAAAAAAAAAAAA"+OPDNo+injectid+injectname+injectAmount);
conn=dbconn.setConnection();

 System.out.println("OOOOO"+conn);
stmt=conn.createStatement();
query= "insert into rhc.patientinject (OPDNo,injectid,injectname,injectamount)"+" values('"+OPDNo+"','"+injectid+"','"+injectname+"','"+injectAmount+"')";
int i=stmt.executeUpdate(query);
 if (i!=0){
 out.println("<script type=\"text/javascript\">");
 out.println("alert ('Injection Sent Succesfully To Injection Room');");
 out.println("location='patientinjection.jsp';");
 out.println("</script>"); 
 }
